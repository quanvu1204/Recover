...
async function uploader() {
  ...
  const PostClient = new PostFinder("gelbooru", currentID);
  ...
  if (currentID > endID) {
    ...
    const ErrorClient = new EmailError(
      `GELBOORU upload script has failed ${failCount.failed_count} times. Investigate for further issues!`
    );
    ...
    console.log(
      `GELBOORU upload script has failed ${failCount.failed_count} times, exiting for now...`
    );
    ...
    const thumbnail = `/home/scripting/Guardian/script/image-store/gelbooru/thumbnail/gelbooru-post_id-${currentID}.jpg`;
    const md5 = post.md5;
    ...
    if (localPost.length === 0) {
      ...
      try {
        await fetch("https://capi-v2.sankakucomplex.com/posts", {
          ...
          .then(async (text) => {
            ...
            if (
              ...
              console.log(
                `Failed to upload GELBOORU#${currentID}. Error(s): ${text}`
              );
        ...
        StorageClient.addRetry();
        console.log(
          `GELBOORU#${currentID} will need to be retried, API error.`
        );
        return (currentID = currentID + 1);
      }
    } else {
      ...
      fetch(`https://capi-v2.sankakucomplex.com/posts/${lPost.id}`, {
        ...
        .then(async (text) => {
          ...
          if (response.success !== true) {
            ...
            console.log(
              `Failed to update GELBOORU#${currentID}. Error(s): ${text}`
            );
            StorageClient.addRetry();
            return (currentID = currentID + 1);
          } else {
...
async function authorizer() {
  const body = {
    login: process.env.AMORPHOUSBLOB_USERNAME,
    password: process.env.AMORPHOUSBLOB_PASSWORD,
  };
