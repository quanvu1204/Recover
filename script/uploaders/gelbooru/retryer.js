...
async function retryer() {
  ...
  if (post === "none") {
    ...
    const ErrorClient = new EmailError(
      `GELBOORU retry script has failed ${failCount.failed_count} times. Investigate for further issues!`
    );
    ...
    console.log(
      `GELBOORU retry script has failed ${failCount.failed_count} times, exiting for now...`
    );

    return clearInterval(retryInterval);
  } else {
    console.log(`Processing GELBOORU#${post.remote_id}`);
    ...
    try {
      ...
      return console.log(
        `An error occurred while trying to retry GELBOORU#${currentID}. Error(s): ${error}`
      );
    }
  }
}
...
if (process.argv[2] !== "stop") {
  ...
  console.log(`GELBOORU retry script started.`);
}
