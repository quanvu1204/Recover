...
async function uploader() {
    ...
    console.log(`Processing remote post E-HENTAI#${currentID}.`);
    ...
    if (currentID > endID) {
        ...
        if (localPost.length === 0) {
            ...
            const body = { post: { parent_id: newParent, rating: lPost.rating, source: lPost.source, tags: newTags } };
            ...
            .then(async text => {
                ...
                if (response.success !== true) {
                    ...
                    const LockingClient = new RatingLocker(response.post.id, response.post.tags.map(tag => tag.name).join(" "), response.post.rating, response.post.status, response.post.tags.filter(tag => tag.type === 0).length, response.post.parent_id, uploaderToken);
...
async function authorizer() {
    ...
    const body = { login: process.env.ANONYMOUS_USERNAME, password: process.env.ANONYMOUS_PASSWORD };
