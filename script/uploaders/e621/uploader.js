...
async function uploader() {
  console.log(`Processing remote post E621#${currentID}.`);

  const PostClient = new PostFinder("e621", currentID);
  const post = await PostClient.findById();
  const PreStatisticClient = new StatisticRetrieving("e621", "fail");
  const failCount = await PreStatisticClient.collect();
  const StorageClient = new StorageSystem("e621", currentID);
  ...
  if (currentID > endID) {
    ...
    console.log(`No data for E621#${currentID} found in database.`);
    return (currentID = currentID + 1);
  } else if (failCount.failed_count > failID) {
    // Initiate email error
    const ErrorClient = new EmailError(
      `E621 upload script has failed ${failCount.failed_count} times. Investigate for further issues!`
    );
    ...
    console.log(
      `E621 upload script has failed ${failCount.failed_count} times, exiting for now...`
    );
    ...
    const thumbnail = `/home/scripting/Guardian/script/image-store/e621/thumbnail/e621-post_id-${currentID}.jpg`;
    const md5 = post.md5;
      ...
      try {
        await fetch("https://capi-v2.sankakucomplex.com/posts", {
          ...
          .then(async (text) => {
            ...
            const StatisticClient = new StatisticTracking("e621");

            if (response.success !== true) {
              var increment = StatisticClient.incrementFailCount();

              console.log(
                `Failed to upload E621#${currentID}. Error(s): ${text}`
              );
              ...
              console.log(`Successfully uploaded E621#${currentID}.`);
        ...
        console.log(error);
        StorageClient.addRetry();
        console.log(`E621#${currentID} will need to be retried, API error.`);
        return (currentID = currentID + 1);
      }
    } else {
      ...
      fetch(`https://capi-v2.sankakucomplex.com/posts/${lPost.id}`, {
        ...
        .then(async (text) => {
          ...
          if (response.success !== true) {
            ...
            console.log(
              `Failed to update E621#${currentID}. Error(s): ${text}`
            );
            StorageClient.addRetry();
            return (currentID = currentID + 1);
          } else {
            ...
            console.log(`Successfully updated E621#${currentID}.`);
            fs.unlink(file, async function () {
              console.log("Successfully purged post from the file system!");
            });
            fs.unlink(thumbnail, async function () {
...
async function authorizer() {
  const body = {
    login: process.env.FURRIER_USERNAME,
    password: process.env.FURRIER_PASSWORD,
  };
...
if (process.argv[2] !== "stop") {
  ...
  console.log(`E621 upload script started with beginning ID ${startID}.`);
}
