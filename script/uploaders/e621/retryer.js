...
async function retryer() {
  const PostClient = new PostFinder("e621");
  ...
  if (post === "none") {
    ...
    const ErrorClient = new EmailError(
      `E621 retry script has failed ${failCount.failed_count} times. Investigate for further issues!`
    );
    ...
    console.log(
      `E621 retry script has failed ${failCount.failed_count} times, exiting for now...`
    );

    return clearInterval(retryInterval);
  } else {
    console.log(`Processing E621#${post.remote_id}`);
    ...
    try {
      ...
      return console.log(
        `An error occurred while trying to retry E621#${currentID}. Error(s): ${error}`
      );
    }
  }
}
...
if (process.argv[2] !== "stop") {
  ...
  console.log(`E621 retry script started.`);
}
