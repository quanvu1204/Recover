...
async function retryer() {
  ...
  if (post === "none") {
    ...
    const ErrorClient = new EmailError(
      `DANBOORU retry script has failed ${failCount.failed_count} times. Investigate for further issues!`
    );
    ...
    console.log(
      `DANBOORU retry script has failed ${failCount.failed_count} times, exiting for now...`
    );

    return clearInterval(retryInterval);
  } else {
    console.log(`Processing DANBOORU#${post.remote_id}`);
    ...
    try {
      ...
      return console.log(
        `An error occurred while trying to retry DANBOORU#${currentID}. Error(s): ${error}`
      );
    }
  }
}
...
if (process.argv[2] !== "stop") {
  ...
  console.log(`DANBOORU retry script started.`);
}
