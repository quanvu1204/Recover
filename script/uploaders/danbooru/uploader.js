...
async function uploader() {
  console.log(`Processing remote post DANBOORU#${currentID}.`);

  const PostClient = new PostFinder("danbooru", currentID);
  const post = await PostClient.findById();
  const PreStatisticClient = new StatisticRetrieving("danbooru", "fail");
  const failCount = await PreStatisticClient.collect();
  const StorageClient = new StorageSystem("danbooru", currentID);
  ...
  if (currentID > endID) {
    ...
    return (endID = latestPosts[0].id - 25);
  } else if (post === "none") {
    console.log(`No data for DANBOORU#${currentID} found in database.`);
    return (currentID = currentID + 1);
  } else if (failCount.failed_count > failID) {
    // Initiate email error
    const ErrorClient = new EmailError(
      `DANBOORU upload script has failed ${failCount.failed_count} times. Investigate for further issues!`
    );
    ...
    console.log(
      `DANBOORU upload script has failed ${failCount.failed_count} times, exiting for now...`
    );
    ...
    const thumbnail = `/home/scripting/Guardian/script/image-store/danbooru/thumbnail/danbooru-post_id-${currentID}.jpg`;
    const md5 = post.md5;
      ...
      try {
        await fetch("https://capi-v2.sankakucomplex.com/posts", {
          ...
          .then(async (text) => {
            ...
            const StatisticClient = new StatisticTracking("danbooru");

            if (response.success !== true) {
              var increment = StatisticClient.incrementFailCount();

              console.log(
                `Failed to upload DANBOORU#${currentID}. Error(s): ${text}`
              );
              ...
              console.log(`Successfully uploaded DANBOORU#${currentID}.`);
        ...
        StorageClient.addRetry();
        console.log(
          `DANBOORU#${currentID} will need to be retried, API error.`
        );
        return (currentID = currentID + 1);
      }
    } else {
      ...
      fetch(`https://capi-v2.sankakucomplex.com/posts/${lPost.id}`, {
        ...
        .then(async (text) => {
          ...
          if (response.success !== true) {
            ...
            console.log(
              `Failed to update DANBOORU#${currentID}. Error(s): ${text}`
            );
            StorageClient.addRetry();
            return (currentID = currentID + 1);
          } else {
            ...
            console.log(`Successfully updated DANBOORU#${currentID}.`);
            fs.unlink(file, async function () {
              console.log("Successfully purged post from the file system!");
            });
            fs.unlink(thumbnail, async function () {
...
async function authorizer() {
  const body = {
    login: process.env.KABESHI_USERNAME,
    password: process.env.KABESHI_PASSWORD,
  };
...
if (process.argv[2] !== "stop") {
  ...
  console.log(`DANBOORU upload script started with beginning ID ${startID}.`);
}
