...
async function uploader() {
    ...
    const PostClient = new PostFinder("danbooru");
    ...
    if (currentID > 7000000) {
        console.log(`DANBOORU upload script has finished.`);
        ...
        const ErrorClient = new EmailError(`DANBOORU upload script has failed ${failCount.failed_count} times. Investigate for further issues!`);
        ...
        console.log(`DANBOORU upload script has failed ${failCount.failed_count} times, exiting for now...`);
          ...
          return StorageClient.deletePost(currentID);
            ...
            try {
                ...
                .then(async text => {
                    ...
                    if (response.success !== true && (response.code && response.code !== "file_uploading_in_another_thread")) {
                        ...
                        console.log(`Failed to upload DANBOORU#${currentID}. Error(s): ${text}`);
                ...
                console.log(error);
                StorageClient.addRetry();
                console.log(`DANBOORU#${currentID} will need to be retried, API error.`);
                return currentID = currentID + 1;
            }
        } else { 
            ...
            .then(async text => {
                ...
                if (response.success !== true) {
                    ...
                    console.log(`Failed to update DANBOORU#${currentID}. Error(s): ${text}`);
...
async function authorizer() {
    ...
    const body = { login: process.env.KABESHI_USERNAME, password: process.env.KABESHI_PASSWORD };
